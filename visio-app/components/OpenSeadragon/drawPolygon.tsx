...
function ensurePolygonState(dragRef: React.MutableRefObject<any>): PolygonLiveState {
...
function imagePointsToViewportPoints(viewer: OpenSeadragon.Viewer, pts: PolygonPoint[]): OpenSeadragon.Point[] {
  return pts.map((p) => viewer.viewport.imageToViewportCoordinates(p.x, p.y));
}

function bboxOfImagePoints(pts: PolygonPoint[]): { x: number; y: number; w: number; h: number } {
  let minX = Infinity,
    minY = Infinity,
    maxX = -Infinity,
    maxY = -Infinity;
...
function setSvgViewBoxToViewportBBox(svgEl: SVGSVGElement, vpts: OpenSeadragon.Point[]): void {
  let minX = Infinity,
    minY = Infinity,
    maxX = -Infinity,
    maxY = -Infinity;
...
function createPolygonOverlaySvg(): {
  ...
  poly.setAttribute("fill", "rgba(255,59,48,0.08)");
  poly.setAttribute("stroke", "#ff3b30");
  poly.setAttribute("stroke-width", "0.0006"); // unités viewport
  poly.setAttribute("stroke-linejoin", "round");

  const preview = document.createElementNS(svgNS, "polyline");
  preview.setAttribute("fill", "none");
  preview.setAttribute("stroke", "#ff3b30");
  preview.setAttribute("stroke-width", "0.0006");
  preview.setAttribute("stroke-dasharray", "0.003 0.003");
...
function updateOverlay(
  ...
  if (!polygonState.svgEl || !polygonState.polyEl || !polygonState.previewEl) return;
  ...
  if (mouseImagePoint && pts.length >= 1) {
    const vMouse = viewer.viewport.imageToViewportCoordinates(mouseImagePoint.x, mouseImagePoint.y);
...
export function polygonCancel(viewer: OpenSeadragon.Viewer, dragRef: React.MutableRefObject<any>): void {
  const poly = ensurePolygonState(dragRef);
  if (!poly.active) return;
